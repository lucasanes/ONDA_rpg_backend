...
} from '@src/domain/usecases/item/find-items-by.usecase';

@Injectable()
export class FindItemsByUsecaseImpl implements FindItemsByUsecase {
  constructor(
  ...
  async execute(
    params: FindItemsByUsecaseInput,
    ...
    if (characterId) {
      ...
      return await this.itemRepository.findBy({ characterId });
    }
    ...
    if (sessionId) {
      ...
      return await this.itemRepository.findBy({ sessionId });
    }
  }
}
